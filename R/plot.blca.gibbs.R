...
function(x, which= c(1L, 3L), main="", ...){
	if(attr(x, "class")!="blca.gibbs") stop("Incorrect class")
	...
	for(m in 1:M){
		matplot(x$samples$itemprob[,,m],  type='l', main=main, sub="Parameter Chains", ylab="Item Probability", xlab="Iteration",...)
		mtext(caption[m], 3, 0.25)		
		
		}
		
	matplot(x$samples$classprob,  type='l', main=main, sub="Parameter Chains", ylab="Class Probability", xlab="Iteration",...)	
		} 
